...
import { ColumnChangesService } from '../../../public-api';
...
describe('DatatableComponent With Custom Templates', () => {
  ...
  it('should reflect changes to input bindings of `bks-datatable-column`', () => {
    const initialRows = [
...
@Component({
  template: ` <bks-datatable [columns]="columns" [rows]="rows" [sorts]="sorts"> </bks-datatable> `
})
...
@Component({
  template: `
    <bks-datatable [rows]="rows" [sorts]="sorts">
      <bks-datatable-column name="Id" prop="id">
        <ng-template let-column="column" bks-datatable-header-template>
          {{ column.name }}
        </ng-template>
        <ng-template let-row="row" bks-datatable-cell-template>
          {{ row.id }}
        </ng-template>
      </bks-datatable-column>
      <bks-datatable-column [prop]="columnTwoProp">
        <ng-template let-column="column" bks-datatable-header-template>
          {{ column.name }}
        </ng-template>
        <ng-template let-row="row" let-column="column" bks-datatable-cell-template>
          {{ row[column.prop] }}
        </ng-template>
      </bks-datatable-column>
    </bks-datatable>
 ...
 * body of the bks-datatable component
 */
