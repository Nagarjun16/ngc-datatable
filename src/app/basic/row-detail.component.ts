...
@Component({
  ...
  template: `
    <div>
      ...
      >
        <!-- Row Detail Template -->
        <bks-datatable-row-detail [rowHeight]="100" #myDetailRow (toggle)="onDetailToggle($event)">
          <ng-template let-row="row" let-expanded="expanded" bks-datatable-row-detail-template>
        ...
        <bks-datatable-column
          ...
          [canAutoResize]="false"
        >
          <ng-template let-row="row" let-expanded="expanded" bks-datatable-cell-template>
            <a
        ...
        <bks-datatable-column name="Index" [width]="80">
          <ng-template let-rowIndex="rowIndex" let-row="row" bks-datatable-cell-template>
            <strong>{{ rowIndex }}</strong>
          </ng-template>
        </bks-datatable-column>
        <bks-datatable-column name="Expanded" [width]="80">
          <ng-template let-row="row" let-expanded="expanded" bks-datatable-cell-template>
            <strong>{{ expanded === 1 }}</strong>
          </ng-template>
        </bks-datatable-column>
        <bks-datatable-column name="Name" [width]="200">
